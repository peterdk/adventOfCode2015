...
def fast_steps_to_molecule(molecule)
input = [0, molecule]
puts fast_steps(input)
...
def fast_steps(molecule)    
    match = molecule[1].reverse.match(/(#{@molecules.map{|a|a.name.reverse}.sort.reverse.join("|")})/)
    
    all, mol = match.to_a       
    if (match)
    replacement = @molecules.find{|a|a.name.reverse == mol}.conversions.first.name.reverse
    output = [molecule[0] + 1, molecule[1].dup.reverse.sub(mol, replacement).reverse]       
    puts output
    if (output[1] == "e")
        return output
    else
        return fast_steps(output)
    end
    end
    molecule
...
p.fast_steps_to_molecule(File.new("input19.txt").read.strip)
